...
迴圈2 :
	...
	若 （ file1 的第一個element 和 lcs 的第一個 element 相同 ） 或（file1 為空）就把 file1 的第一個 elemt pop 掉
	並且跳出迴圈 2
...
迴圈 3 :
	...
	若 （ file2 的第一個element 和 lcs 的第一個 element 相同 ） 或（file2 為空）就把 file2 的第一個 elemt pop 掉
	然後印出 lcs 的第一個 element , 並且把 lcs 的第一個 element pop 掉
	最後跳出迴圈 3
